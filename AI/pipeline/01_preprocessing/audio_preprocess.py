...
try:
    ...
    raise ImportError(
        "audio_preprocess requires numpy, librosa and soundfile. "
...
def split_to_bars(
    ...
    if bars:
        bar_aligned = np.concatenate(bars).astype(np.float32)
    else:
        bar_aligned = np.empty(0, dtype=np.float32)
