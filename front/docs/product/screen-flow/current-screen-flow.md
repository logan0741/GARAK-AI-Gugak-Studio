...
| S07 | 트랙/레이어 편집 | 자동 저장된 작업을 열고 여러 테이크와 반주를 박자 그리드 위에 레이어로 쌓아 편집한다. | 작업 저장, 내보내기, 트랙 추가, 다시 녹음 | S05, S18 | S08, S17, S18 | 필수 |
...
- 내보내기 성공 -> S17 `공유 준비` 또는 S18 `보관함`
- 보관함에서 작업 열기 -> S07

#### 14. 논의 필요사항

- 작업 자동 저장이 보관함에 즉시 노출될지, `최근 작업` 영역에만 먼저 노출될지 정해야 한다.
- 이름 없는 작업이 쌓일 때의 정리 정책을 정해야 한다.
- 세밀한 위치 조정을 숙련자용/전문가용 기능으로 열 경우, 어떤 진입 방식과 표시 단위를 쓸지 정해야 한다.
- 내보내기 완료 후 바로 공유 준비로 보낼지, 보관함에 저장한 뒤 상세에서 공유하게 할지 정해야 한다.
...
- 악기 연주 추가 후 S04A 수준의 악기별 기본 설정을 다시 보여줄지, 기본값으로 바로 S09에 진입할지 정해야 한다.
- 가져오기 잠금 문구와 노출 방식을 정해야 한다.
...
- 추가 녹음 트랙의 시작 위치를 녹음 시작 시점 기준으로 둘지, 현재 재생 헤드 기준으로 둘지 정해야 한다.
- 기존 트랙을 들을 때 전체 트랙을 항상 재생할지, mute/solo 상태를 S07에서 그대로 가져올지 정해야 한다.
...
- S10A에서 적용한 라이브 장단을 테이크 메타데이터에 어떻게 남길지 정해야 한다.
...
- S10B의 반주 트랙 시작 위치를 작업 시작점으로 둘지, 현재 재생 헤드 위치로 둘지 정해야 한다.
- S10B에서 AI 추천 장단을 바로 보여줄지, 수동 프리셋 선택을 먼저 둘지 정해야 한다.
