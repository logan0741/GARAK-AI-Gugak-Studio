...
flowchart TD
    ...
    FreeMode --> InstrumentSettings["악기 선택 및 조율"]
    InstrumentSettings --> FreePlay["악기 자유연주"]
...
| S04 | 악기 선택 및 조율 설정 | 자유창작 모드에서 가야금, 장구, 대금 중 연주할 악기를 고르고 필요한 조율/터치/잔향 설정을 확인한다. 추후 악기 확장 슬롯은 잠금 상태로 보여준다. | 악기 선택, 설정 저장하기, Next | S01, S05 | S05 또는 이전 화면 | 필수 |
...
- `자유창작 모드`를 선택하고 `Next`를 누르면 S04 `악기 선택 및 조율 설정`으로 이동한다.
...
- 사용자가 가야금, 장구, 대금 중 하나를 선택하면 해당 악기의 설명과 기본 설정을 확인한다.
...
- 장구 선택
- 대금 선택
- Next: 선택한 악기의 S05 `악기 자유연주` 화면으로 이동한다.
...
- 잠금 슬롯에 실제 악기명을 미리 보여줄지, 악기명 없이 잠금 아이콘만 둘지 정해야 한다.
- 잠금 슬롯을 눌렀을 때 아무 반응이 없게 둘지, 짧은 안내를 보여줄지 정해야 한다.
- 조율/터치/잔향 설정을 S04에서 모두 보여줄지, 상세 설정은 연주 화면 내부 설정으로 넘길지 정해야 한다.
