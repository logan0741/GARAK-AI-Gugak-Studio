...
| File | Responsibility |
| --- | --- |
| `src/audio/expoAudioSamplerEngine.ts` | Candidate A `SamplerEngine` implementation. Owns source resolution before preload, all-or-nothing player activation after downloaded URI validation, finite playback-control validation before native player mutation, pluck playback, queued playback failure reporting through `waitForIdle()`, release ordering behind pending seek/play work, bend approximation, mute/release mapping, recording probe lifecycle, non-empty captured recording URI validation, and captured recording URI playback. |
...
| `src/prototype/prototypeRecordingProbeController.ts` | Prototype boundary that calls optional recording probe methods, rejects invalid requested or native start durations, normalizes empty captured URIs to `null`, and reports unsupported, failed, recording, captured, missing-URI, or playback states without breaking session fallback. |
...
Resolve every `SampleAssetManifest.fileUri` through the prototype bundled sample registry and Expo Asset before constructing `ExpoAudioSamplerEngine`. Do not use remote URLs for normal-play latency checks. The native sampler factory rejects empty or `http(s)` source `fileUri` values before asset resolution, and rejects empty or `http(s)` resolved URIs before loading the candidate runtime. During preload, each downloaded Expo Audio source URI must still be non-empty before any player is created; a blank downloaded URI is a candidate preload failure, not Day 5 evidence, and must not leave partially playable strings. If any string sample is missing, duplicated, or outside the 1-12 prototype range, the prototype host must stay on `fake-prototype` and report the affected string indexes; the native sampler factory also rejects missing, duplicate, or unexpected string indexes before creating a candidate runtime. If the manifest is complete and has exactly one asset for each 1-12 string but native preload has not finished, the prototype host must show `native_candidate_preloading` and keep dispatching to the fake fallback until preload succeeds.
...
Treat a `null`, empty, or whitespace-only `recordingUri` as missing playback context. The Expo Audio stop result must normalize it to `null`; the inspector must keep `uriAvailable: false` and record `fallbackReason: "recording_playback_uri_missing"` instead of enabling captured playback. A zero or invalid captured duration must be recorded as `fallbackReason: "recording_capture_duration_invalid"` and must not expose `Play Rec` or allow playback confirmation even when a URI is present. A native recording failure with a blank reason must be normalized to `fallbackReason: "recording_probe_failed"` before it reaches the inspector. A new capture must clear any previous `playbackConfirmed: true`; a recording fallback must clear previous `capturedSeconds`, `uriAvailable`, and `playbackConfirmed` so stale capture evidence is not copied. Replay the current captured URI before copying playback evidence. A native recording start response must echo a positive finite `requestedDurationSeconds`; otherwise the probe must stay failed and must not enter the `recording` state.
