...
For candidates that cannot capture audio, keep the `Session fallback` JSON and copy `observedPrototypeRecording.fallbackReason` from the prototype draft. Blank native fallback reasons are normalized to `recording_probe_failed` before they reach the inspector. If a filled prototype handoff uses `recordingCaptureSeconds: 0`, the matching inspector draft must include a non-empty `observedPrototypeRecording.fallbackReason`; otherwise `qa:prototype-handoff-check` reports `recordingCaptureSeconds` as invalid. That reason is handoff context only; final selection still depends on the manually reviewed `physical-device` probe record.

The `Session fallback` formatter trims captured recording URIs in both the MVP compatibility `recordingUri` field and `recordings[]`, and it omits blank or whitespace-only recording outputs before producing copyable JSON.
