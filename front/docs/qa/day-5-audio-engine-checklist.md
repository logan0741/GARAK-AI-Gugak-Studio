...
After copying `Session fallback` JSON to a file, validate it with `npm run qa:session-fallback -- <session-fallback.json> [sample-manifest.json]`. The command must report `REPLAYABLE_SESSION_FALLBACK` before the fallback is treated as preserved replay evidence. When a sample manifest is provided, the command must also prove `ReplayPlanner` can build a replay schedule from the copied session and matching manifest.

The prototype inspector must also expose `Session replay` as a compact readiness summary. It should show waiting before any replayable events, ready when `ReplayPlanner` can schedule the current session against the prototype manifest, and blocked when manifest or sample validation prevents deterministic replay.
...
For D-2 scoped demo evidence, an `expo-audio` physical-device row may be used before the two-candidate Day 5 decision is complete only when that row evaluates to `PASS` or `PASS_WITH_LIMITS`. A non-zero `qa:day5-audio` exit is acceptable for the D-2 note only when the remaining blocker is the missing `react-native-audio-api` physical-device probe, not when the `expo-audio` probe itself fails.
...
Then run the handoff object through `npm run qa:day5-audio -- <probe-record.json>`. The handoff must return parse errors for invalid records, or a formatted Day 5 decision summary for valid records. The command exits non-zero unless the summary reaches `FINAL_ENGINE_SELECTED`; `INCOMPLETE_DEVICE_EVIDENCE`, `NO_FINAL_ENGINE`, and invalid records must block the gate. The record must remain `INCOMPLETE_DEVICE_EVIDENCE` until both `expo-audio` and `react-native-audio-api` have `evidenceSource: 'physical-device'` probe values, no candidate appears more than once, and all physical-device probes use one device label after trimming whitespace and normalizing slash spacing.
...
17. Confirm `Session replay` shows `Replay waiting: 0 events` before any event, then `Replay ready` with finite event count and duration after a tap, glissando, bend, mute, or polyphony probe event is appended.
18. Press `Replay` after `Session replay` is ready and confirm `Session replay dispatch` reports the replayed event count. Treat this as replay-path smoke only; it must not be copied as physical-device latency, polyphony, pitch-bend, mute, or recording evidence.
19. Press `Glissando` and confirm the event count increments by 12.
20. Press `8 Voice` and confirm the event count increments by 8. On device, listen for dropout or voice stealing; on web fake runtime, confirm audible fake voice count reaches at least 8 before release cleanup.
21. While active runtime is `fake-prototype`, confirm audible fake voice count grows for plucks and does not count released voices.
22. Confirm audio status remains `ok` while the current engine handles events. If a batch fails, confirm it reports handled event count, total event count, failed event index, error message, and failed `PerformanceEvent` while the session fallback still preserves the full event batch.
23. Confirm the probe draft exposes `observedFakeCounters.eventDispatchLatency` after at least one handled event batch, and keep `probeTemplate.touchToSoundLatencyMs` as `null` until physical-device audio latency is measured.
24. Confirm the `Session fallback (copyable)` JSON uses format `gukak-studio-session-fallback-v1`, has `canReplay: true` after at least one event, preserves the full `Session.events` list even if recording is unsupported or fails, trims captured recording URIs, omits null, empty, or whitespace-only recording URIs, and passes `npm run qa:session-fallback -- <session-fallback.json> [sample-manifest.json]`.
