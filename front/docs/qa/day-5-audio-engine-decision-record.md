...
| `src/prototype/prototypeProbeHandoff.ts` | Converts prototype inspector drafts into physical-device probes or a Day 5 probe record only when estimate-only inspector guard fields are intact and observed runtime context proves each requested native candidate is ready and uses the Week 1 fixture sample manifest. |
