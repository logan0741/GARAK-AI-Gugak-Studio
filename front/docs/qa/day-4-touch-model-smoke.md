...
| `src/prototype/gayageumPrototypeController.ts` | Plans deterministic tap, glissando, bend, mute, and 8-voice probe event batches, appends them to the replayable session, plans current-session replay dispatch, dispatches replay schedules through the current `SamplerEngine`, and reports how many events reached the engine before any dispatch failure. |
...
| `src/prototype/prototypeSessionReplayPreview.ts` | Summarizes whether the current session can be scheduled for deterministic replay with the prototype sample manifest. This is UI readiness feedback, not physical-device audio evidence. |
