...
이 문서는 GUKAK STUDIO 프론트엔드 작업의 협업 컨벤션이다. 커밋, 브랜치 생성, 코드 구현은 이 문서를 따른다.
...
프론트엔드 작업 산출물은 저장소의 `front/` 하위에 둔다. 현재 GUKAK STUDIO 프론트엔드 프로젝트의 문서, 앱 코드, 패키지 설정은 `front/` 아래에서 관리한다.
