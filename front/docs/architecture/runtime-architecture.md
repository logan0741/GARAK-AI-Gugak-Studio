...
| Session replay requested | Build a deterministic `ReplaySchedule` from `Session.events` and the matching `SampleAssetManifest`, then dispatch its events through the current `SamplerEngine` boundary. |
