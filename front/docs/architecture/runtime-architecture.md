...
| Session replay requested | Build a deterministic `ReplaySchedule` from `Session.events` and the matching `SampleAssetManifest`, preview readiness in the prototype inspector, then dispatch its events through the current `SamplerEngine` boundary. |
