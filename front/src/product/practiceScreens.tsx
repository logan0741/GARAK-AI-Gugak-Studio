...
export function PracticePerformanceContent({
    ...
    practiceStatus === 'playing'
      ...
      : practiceStatus === 'paused'
        ...
        : practiceStatus === 'completed'
          ? '완주 완료'
          : '가이드 준비 완료';
  const inputCount = practiceAttempt?.inputEvents.length ?? 0;
  const guideTotal = practiceAttempt?.guideEvents.length ?? 6;
  const activeGuideCell = inputCount % 6;
  const practiceGuideText =
    practiceStatus === 'paused'
      ? '다시 시작하면 처음부터 기록합니다.'
      : `현재 위치 ${inputCount}/${guideTotal} · 다음 입력 ${getInstrumentName(instrument)}`;
  ...
  return (
    <View style={[styles.stack, isLandscapeFrame ? styles.landscapePerformanceStack : undefined]}>
      ...
      <Pressable
        accessibilityLabel="연습 입력 기록"
        accessibilityRole="button"
        disabled={practiceStatus !== 'playing'}
        onPress={() => dispatch({
          type: 'recordPracticeInput',
          stringIndex: practiceAttempt?.guideEvents[practiceAttempt.inputEvents.length]?.stringIndex,
        })}
        style={[styles.practiceSurface, isLandscapeFrame ? styles.landscapePracticeSurface : undefined]}
      >
        ...
        <View style={styles.guideRow}>
          {Array.from({ length: 6 }, (_, index) => (
            <View
              ...
              style={[
                styles.guideCell,
                practiceStatus === 'playing' && index === activeGuideCell
                  ? styles.guideCellActive
                  : index < activeGuideCell
                    ? styles.guideCellDone
                    : undefined,
              ]}
            />
          ))}
