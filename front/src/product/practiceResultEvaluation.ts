...
const NO_ATTEMPT_ACCURACY_SCORE = 0;
const NO_ATTEMPT_TIMING_SCORE = 0;
const NO_ATTEMPT_FEEDBACK = '연주 입력이 기록되지 않았어요.';
...
export function buildPracticeResultFeedback(
  ...
  if (evaluation.resultKind === 'localTemplate') {
    return {
      title: NO_ATTEMPT_FEEDBACK,
      description: '연주 이벤트가 있어야 정확도와 박자 점수를 계산할 수 있어요.',
      fullText: NO_ATTEMPT_FEEDBACK,
    };
  }
...
function scorePracticeAttempt(attempt: PracticeResultAttemptInput | undefined): {
  ...
  if (inputEvents.length === 0 && timingErrorsMs.length === 0) {
    return {
      accuracyScore: NO_ATTEMPT_ACCURACY_SCORE,
      timingScore: NO_ATTEMPT_TIMING_SCORE,
