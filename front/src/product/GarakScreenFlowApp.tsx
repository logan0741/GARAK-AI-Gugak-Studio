...
import { ExpoAudioSamplerEngine } from '../audio/expoAudioSamplerEngine';
...
export function GarakScreenFlowApp({
  ...
  useEffect(() => {
    const manifest = sampleManifests?.gayageum;
    if (!manifest) {
      return;
    }

    const engine = new ExpoAudioSamplerEngine({
      manifest,
      runtime: createExpoAudioRuntimePort(),
    });

    samplerEngineRef.current = engine;
    void engine.preload().catch(() => {
      if (samplerEngineRef.current === engine) {
        samplerEngineRef.current = null;
      }
    });
  ...
  const dispatch = useCallback((action: GarakProductAction) => {
    if (action.type === 'appendFreePlayPerformanceEvents' && samplerEngineRef.current !== null) {
      for (const event of action.events) {
        try {
          samplerEngineRef.current.handleEvent(event);
        } catch {
          // Engine errors must not interrupt state updates
        }
      }
    }
