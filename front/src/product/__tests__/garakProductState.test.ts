...
test('summarizes S13 song selection with preview from the detailed document', () => {
  let state = createInitialGarakProductState();
  ...
  expect(state.screenFlow.currentScreen).toBe('S13');
  expect(getCurrentScreenSummary(state).primaryCtas).toEqual(
    expect.arrayContaining(['아리랑', '도라지', '뱃노래', '미리듣기']),
  );
});
...
test('routes S16 retry and choose-another-song actions through connected practice result actions', () => {
  ...
  expect(retryState.practiceAttempt).toMatchObject({
    songId: 'doraji',
    instrument: 'janggu',
    status: 'ready',
    inputEvents: [],
...
test('handles S19 favorite, previous, next, and AirPlay controls', () => {
  let state = createInitialGarakProductState({
    now: () => '2026-06-18T00:00:00.000Z',
  });

  // Inject two real library items to test next/previous navigation.
  state = applyProductAction(state, {
    ...
    library: {
      ...
      exportedAudios: [
        ...
        },
        {
          ...
          audioUri: 'file://falling-water.aac',
          shareState: 'ready',
        },
      ],
      practiceResults: [],
  ...
  state = applyProductAction(state, {
    type: 'playLibraryItem',
    item: { kind: 'exportedAudio', exportedAudioId: 'export-a' },
  });
  state = applyProductAction(state, { type: 'toggleSelectedPlayerFavorite' });

  expect(state.favoritePlayerItems).toEqual(['exportedAudio:export-a']);

  state = applyProductAction(state, { type: 'playNextPlayerItem' });

  expect(state.selectedPlayerItem).toEqual({
    kind: 'exportedAudio',
    exportedAudioId: 'export-b',
  });

  state = applyProductAction(state, { type: 'playPreviousPlayerItem' });
  state = applyProductAction(state, { type: 'activateAirPlay' });

  expect(state.selectedPlayerItem).toEqual({
    kind: 'exportedAudio',
    exportedAudioId: 'export-a',
  });
  expect(state.playerNotice).toBe('airPlayReady');
});
...
test('records S15 practice taps as performance events for AI feedback input', () => {
  const timestamps = [
    '2026-06-18T00:00:00.000Z',
    '2026-06-18T00:00:00.520Z',
  ];
  let nowIndex = 0;
  let state = createInitialGarakProductState({
  ...
  state = applyProductAction(state, { type: 'next' });
  state = applyProductAction(state, { type: 'selectPracticeSong', songId: 'arirang' });
  state = applyProductAction(state, { type: 'selectPracticeInstrument', instrument: 'gayageum' });
  state = applyProductAction(state, { type: 'next' });
  state = applyProductAction(state, { type: 'startPractice' });
  nowIndex = 1;
  state = applyProductAction(state, { type: 'recordPracticeInput' });

  // 아리랑 첫 가이드 이벤트: tsMs=0, stringIndex=5
  // 타이밍 오차 = 입력 tsMs(520) - 가이드 tsMs(0) = 520
  expect(state.practiceAttempt?.inputEvents).toEqual([
    {
      type: 'string_pluck',
      tsMs: 520,
      stringIndex: 5,
  ...
  expect(state.practiceAttempt?.timingErrorsMs).toEqual([520]);

  state = applyProductAction(state, { type: 'finishPractice' });

  expect(state.practiceAiFeedback).toEqual({ status: 'loading' });
});
