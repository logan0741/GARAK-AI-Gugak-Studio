import { PerformanceEvent, createStringPluck } from '../domain/performanceEvent';
import {
...
export type ProductPlayerNotice = 'airPlayReady';
...
export function applyProductAction(
  ...
  switch (action.type) {
    ...
    case 'recordPracticeInput':
      return recordPracticeInput(state, action.stringIndex);
    case 'pausePractice':
...
function finishPractice(state: GarakProductState): GarakProductState {
  ...
  return {
    ...state,
    practiceAttempt: completedAttempt,
    practiceAiFeedback: completedAttempt.inputEvents.length > 0 ? { status: 'loading' } : undefined,
    screenFlow:
