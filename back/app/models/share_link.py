...
class ShareLink(Base):
    ...
    recording_id: Mapped[Optional[str]] = mapped_column(VARCHAR(64), ForeignKey("recording.id", ondelete="SET NULL"), nullable=True)
    ...
    expires_at_ms: Mapped[Optional[int]] = mapped_column(BIGINT(unsigned=True), nullable=True)

    session: Mapped["Session"] = relationship(back_populates="share_links")
    recording: Mapped["Optional[Recording]"] = relationship(back_populates="share_links")
